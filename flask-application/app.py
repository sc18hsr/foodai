...
#loading our recipe dataset
df = pd.read_csv('recipe_data/RAW_recipes.csv')
df = df.dropna()
...
def detail():
    ...
    if id:
        # extract the integer value from the id parameter
        id = ''.join(filter(str.isdigit, id))

        # convert the id to an integer
        id = int(id)

        # get the row for the selected id
        row = df[df['id'] == id].iloc[0]

        # extract the name, ingredients, and steps
        name = row['name']
        ingredients = row['ingredients']
        steps = row['steps']

        return render_template('detail.html', id=id, name=name, ingredients=ingredients, steps=steps)
    else:
